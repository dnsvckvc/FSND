...
- General
    - Returns a set of questions per specific category ID
    - Request arguments: category ID in request URL
- Sample: `curl http://127.0.0.1:5000/categories/1/question`
```
{
  "categories": {
    "1": "Science", 
...
```
- Sample 2: `curl --header "Content-Type: application/json" -d '{"searchTerm":"peanut"}' -X POST http://127.0.0.1:5000/question`
```
{
  "questions": [
    {
...
- General
    - Allows to delete a question by question ID
...
#### POST '/quizzes'
- General
    - Returns a question for quiz playing
    - Additional parameters: in JSON body category ID and previously playes question ids in list for`
Sample: `curl --header "Content-Type: application/json" -d '{"previous_questions": [1], "quiz_category": {"id": 1, "category": "lalala"}}' -X POST http://127.0.0.1:5000/quizzes`
